  ...
  return {
    ...
    html: `
      <div style="font-family:Arial,sans-serif;max-width:600px;margin:auto;color:#163300;line-height:1.55">
        ...
        <p style="font-size:13px;color:#687764">ehllo keeps the event connected to the people you meet, your private captures and the follow-ups you choose. Organisers and attendees cannot see your private notes.</p>
      </div>`,
  };
}
...
export function buildEventScheduleChangedEmail(input: {
  ...
  return {
    ...
    html: emailFrame("Event updated", title, `The event is now scheduled for <strong>${escapeHtml(when)}</strong>${input.location?.trim() ? `<br>Location: ${escapeHtml(input.location.trim())}` : ""}. Your RSVP has been kept.`, "Open your original ehllo invitation if you want to change your response."),
  };
}
