...
-- The connection page is the thread. Both parties see the same one.
--
-- get_shared_history (202608180900) modelled this as a separate "what you both
-- see" list sitting next to a private one, which was wrong: the separation
-- implies the rest is hidden, and the point is that A and B are looking at the
-- same conversation. This replaces it.
--
-- Visibility is per item, by participation, keyed on email - not per workspace.
-- With four people around a meeting, an item reaches whoever it concerns: all
-- four see a meeting they were all at, and a follow-up recorded against three
-- of their addresses reaches exactly those three. Nobody sees an item their
-- address is not on.
--
-- The privacy line is not invented here, it is already in the columns.
-- encounters distinguishes transcript and private_notes from shared_summary,
-- and carries a 'shared' status for the deliberate act of sharing. So:
--
--   owner        their own record, unchanged
--   participant  that the meeting happened, when, and where; plus
--                shared_summary only once status = 'shared'
--   nobody       transcript or private_notes, ever, through this function
--
-- Requiring 'shared' rather than 'reviewed' is DEC-028: reviewing a meeting is
-- not the same act as approving it for the other person to read.
...
revoke all on function public.get_connection_thread(uuid) from public, anon;
grant execute on function public.get_connection_thread(uuid) to authenticated;

-- One truth, not two. The separate "shared history" idea is gone.
drop function if exists public.get_shared_history(uuid);
