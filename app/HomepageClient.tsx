...
export default function HomepageClient({ includeDraftReviewLink = false }: HomepageClientProps) {
  return <main className="homepage-draft">
    <DraftMotion />
    {includeDraftReviewLink ? (
      <div className="draft-review-bar">
        <strong>Homepage Draft Variation</strong>
        <span>Exploration only · current homepage unchanged</span>
    ...
    <footer><div><BrandMark className="brand-mark" size={38}/><strong>ehllo</strong></div><p>Meet someone. Remember what mattered. Make the next move.</p><LinkButton variant="ghost" href="/auth?next=/onboarding">Start for free <ArrowUpIcon size={15}/></LinkButton><p className="footer-links"><a href="/activity">Activity log</a> · <a href="/privacy">Privacy</a> · <a href="/terms">Terms</a></p></footer>
  </main>;
}
