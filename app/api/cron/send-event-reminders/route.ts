...
type DueInvitation = {
  ...
  events: { title: string; location: string; starts_at: string; status: string } | null;
};
...
export async function GET(request: Request) {
  ...
  const { data, error } = await service.from("event_invitations")
    .select("id, invited_email, events!inner(title, location, starts_at, status)")
  ...
  return NextResponse.json({ ok: true, scanned: data?.length ?? 0, sent, failed });
}
